...
class MooncakeAsyncKVManager(MooncakeKVManager):
    def __init__(
        ...
        self._almost_finish_event = torch.cuda.Event()
        self._start_time_dict: Dict[int, float] = {}
    ...
    def mark_layer_ready(self, layer_id: int):
        ...
        if self._current_kv_chunk_infos:
            ...
            # if the model is almost finished
            if layer_id == int(self._kv_cache_nlayers * 0.8):
                self._almost_finish_event.record(torch.cuda.current_stream())
    ...
    def _flush_all_layers(self, rid : int, is_last : bool):
        ...
        if is_last:
            ...
            time_start = self._start_time_dict.pop(rid)
            logger.info(f"finish send (rid={rid}, n_blocks={len(bids)}) in {1000*(time.time() - start_time)} ms, total use {time.perf_counter() - time_start} s.")
    ...
    def prepare_batch(self, sch : "Scheduler", batch : "ScheduleBatch"):
        ...
        for req in batch.reqs:
            ...
            self._start_time_dict[bootstrap_room] = req.queue_time_start
            logger.info(f"prepare_batch {bootstrap_room} use {time.perf_counter() - req.queue_time_start} s.")
    ...
    def transfer_worker(
        ...
        while True:
            try:
                ...
                # we have to make sure the kv_chunk.room is in self.transfer_infos
                assert (kv_chunk.room in self.transfer_infos)
                polls = []
                dst_ranks_infos = []
...
class MooncakeAsyncKVSender(MooncakeKVSender):
    def __init__(self, mgr, bootstrap_addr:str, bootstrap_room:int):
        ...
        # we set waitingForInput status to avoid the case where the decode instance is not ready, but we want to run the model anyway
